...
void FindMins(List<Tree> forest, /* Намира двата най-редки елемента */
...
{ /* Построяване на таблица на честотите на срещане */
  ...
  foreach(var ch in freqs.Keys) {
      forest.Add(new Tree { Char = ch, Freq = freqs[ch], Weight = freqs[ch] });
  }
  ...
  while (forest.Count > 1) {
    ...
    var tree = new Tree { Left = forest[i], Right = forest[j], 
