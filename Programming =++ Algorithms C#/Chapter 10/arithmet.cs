...
Dictionary<char,int> GetStatistics(string msg) /* Намира броя на срещанията на всеки символ */
{ return msg.GroupBy(ch => ch).ToDictionary(g => g.Key, g => g.Count());
}

Dictionary<char, Symbol> BuildModel(string msg, Dictionary<char,int> freqs) /* Построява модела */
{ double lastHigh = 0.0;
...
void PrintModel(Dictionary<char,Symbol> symbols)
{ 
...
double ArithmeticEncode(string msg, Dictionary<char, Symbol> symbols) /* Извършва аритметично кодиране */
...
Symbol GetSymbol(double enc, Dictionary<char, Symbol> symbols)
{ return symbols.Values.Where(sym => sym.Low <= enc && sym.High > enc)
            .First();
  
}

string ArithmeticDecode(double enc, int length, Dictionary<char, Symbol> symbols) /* Извършва декодиране */
{ double range;
...
void Main() {
  ...
  if (SHOW_MORE) {
    PrintModel(symbols);
    Console.Write("Натиснете <<ENTER>>"); Console.ReadLine();
  }
  ...
  Console.WriteLine("Декодиране: {0}", decoded);
}