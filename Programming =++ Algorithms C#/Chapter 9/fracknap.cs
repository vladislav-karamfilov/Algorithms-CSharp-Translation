...
void Sort(double[] ratio, double[] value, double[] quant)
...
/* намира решението */
void Solve(double M, double[] value, double[] quant)
{ int i = 0;
  double T = 0, V = 0;
  while (T + quant[i] <= M) {     /* взима цели предмети, докато може */
...
void Main() {
  ...
  double[] ratio = value.Zip(quant, (v, q) => v / q).ToArray();
